...
| columns    | int \| str \| Mapping\[str, Any\] | Grid column definition. Pass an int for fixed columns, a ``{"minWidth": int, "repeat": "fit" \| "fill", "max": int}`` mapping for responsive columns, or the equivalent CSS string ``"repeat(auto-fit, minmax(280px, 1fr))"``. Other CSS strings raise a ``TypeError`` because Astryx cannot render them. | `2`       |
