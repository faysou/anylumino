...
class ChartPanel(VBox):
    def __init__(self, chart):
        self.chart = chart
        toolbar = Toolbar(
            [{"id": "fit", "label": "Fit", "icon": "arrows-to-circle"}],
            callbacks={"fit": lambda _id: self.fit()},
        )
        super().__init__({"toolbar": toolbar, "chart": chart.widget}, height=520)
...
class ChartWrapper:
    def __init__(self, chart):
        self.chart = chart
        self.widget = chart.widget

    def fit(self):
        self.chart.fit()


tabs = TabPanel({"price": ChartWrapper(chart)})

tabs.get_widget("price")  # chart.widget
tabs["price"].fit()       # wrapper.fit()
tabs.call_owner("price", "fit")
```
