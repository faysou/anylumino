...
combine anywidgets, native anylumino controls, charts, tables, and outputs into one
...
Action widget icons render through Web Awesome's `wa-icon` element, matching
native anylumino controls such as `Button(icon="check")`. Use Web Awesome icon
names and add `icon_family`, `icon_variant`, or `icon_library` for non-default
icon sources. Common names include `plus`, `rotate`, `comment`, `chart-line`,
`forward-step`, `expand`, `circle-half-stroke`, `check`, and `xmark`.
